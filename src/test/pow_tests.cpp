...
/* Test calculation of next difficulty target with adaptive work algorithm */
BOOST_AUTO_TEST_CASE(get_next_work)
{
    ...
    BOOST_CHECK_EQUAL(GetNextWorkRequired(blockIndexLast, &blockHeader, chainParams->GetConsensus()), 0x1e0fffffU); // adaptive algorithm falls back to powLimit when history window is incomplete
    ...
    blockHeader.nTime = 1408732505; // Block #123457
    BOOST_CHECK_EQUAL(GetNextWorkRequired(blockIndexLast, &blockHeader, chainParamsDev->GetConsensus()), 0x207fffffU); // adaptive algorithm falls back to powLimit when history window is incomplete
